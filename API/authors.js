...
//GET - returns array of all authors from database
...
//POST - creates new author as provided by request body
...
//GET by ID - returns a single auther by ID
router.get("/:id", async (req, res, next) => {
  try {
    ...
    if (!author) {
      return next({
        status: 404,
        message: `Could not find author by ${id}.`,
      });
    }
...
//UPDATE - overwrites specified author as provided by request body
router.put("/:id", async (req, res, next) => {
  try {
    const id = +req.params.id;

    //first check of the author exists
    ...
    //then validates the request body
    const { name } = req.body;
    if (!name) {
      return next({
        status: 404,
